...
&nbsp;&nbsp; [2-2. TLX 입력 및 처리](#-tlx-입력-및-처리)  

&nbsp;&nbsp; [2-3. PCA 분석 및 모델화](#-pca-분석-및-모델화)  

&nbsp;&nbsp; [2-4. LLM 연동 및 조언 생성](#-llm-연동-및-조언-생성)  
...
예컨대 AI 기반 자연어처리(NLP)가 없다면, 컴퓨터는 텍스트를 단순한 바이트 배열로만 인식하며 의미를 고려하지 못합니다. <br>
...
미국항공우주국(NASA)은 '비행사의 자동/수동 조종 간 부담 차이'를 비교하기 위해 <strong>작업 부하 지수(TLX)</strong>를 개발했습니다. <br>
...
위와 같은 6가지 항목으로 나누고, 사용자는 각 항목을 **0~100 척도**로 평가하게 됩니다.
...
주성분 분석(PCA)은 다차원 데이터의 차원을 축소시키면서, 데이터의 중요한 정보를 최대한 보존하는 통계 기법입니다. <br>

여기서 중요한 정보란 <strong>데이터들이 흩어져 있는 방향과 정도(=분산)</strong>를 뜻합니다. <br>

따라서 데이터가 가장 넓게 퍼져 있는 방향을 첫 번째 주성분(PC1) 으로 잡고, <br>

PC1과 직교하면서 그다음으로 데이터가 퍼진 정도가 큰 방향을 두 번째 주성분(PC2)으로 잡습니다. <br>

PCA의 원리는 <strong>"PC1과 PC2만으로도 데이터의 큰 흐름을 이해할 수 있다"</strong>이기 때문에  <br> 

주성분 위에 다차원 데이터를 투영(projection)시킴으로써 정보를 더 적은 차원으로 압축시키게 됩니다.
...
> 이를 통해 덜 중요한 변동은 줄이고 **영향도가 큰 조합**을 선명하게 드러냄으로써, <br><br>
...
대규모 언어 모델(LLM)은 방대한 텍스트 데이터로 학습된 AI 모델로 ChatGPT, Claude 등이 대표적인 예입니다. <br> 

일반적으로 질문에 답하거나 요약·분류·생성 등 다양한 언어 관련 작업을 수행할 수 있습니다.  

<img src="./images/llm.png" width="720" alt="LLM" />

- Pre-trained (사전학습) 모델 / Multimodal 모델 (텍스트+이미지+음성 처리) / Fine-tuning (특정 도메인 재학습) 

- 최근에는 별도의 학습 환경을 구축하지 않아도, **API 형태**로 손쉽게 호출 가능하도록 서비스를 제공합니다.
